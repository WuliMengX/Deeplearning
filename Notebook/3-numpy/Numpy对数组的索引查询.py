    ...
    主要针对一维数组和二维数组：
        ...
        神奇索引：用索引里嵌套列表的形式索引单个行和列 了解即可  
            ...
            实例：获取数组中最大的前N个数组
              ...
              # 取最大值对应的4个下标，返回的是index
                arr.argsort()[-4:] # [6, 5, 1, 9]
              
                arr[arr.argsort()[-4:]] # [37, 69, 79, 85]
              
            二维数组：
                Array[[x,y]]       筛选多行，列可以省略，相当于Array[[x,y],:]取x行和y行所有的列
        ...
        猜想：布尔数组应该也有储存True或False对应位置的原数组的元素 执行X[布尔数组]时能索引到原数组对应的元素以进行操作
